...
import { Search, Filter, Music, Star, ArrowRight, Globe } from 'lucide-react';
 ...
 * Displays all traditional music from all countries with filtering and search
 */
export default function MusicPage() {
  ...
  // Extract all music from all countries
  const allMusic = useMemo(() => {
    const music = [];
    Object.entries(CountryDetails).forEach(([countryCode, countryData]) => {
      if (countryData.culture?.music) {
        countryData.culture.music.forEach(genre => {
          music.push({
            name: genre,
            type: 'Musical Genre',
            ...
            description: `Traditional ${genre.toLowerCase()} music from ${countryData.name}, showcasing the unique rhythmic patterns and melodic traditions of the region.`,
            origin: countryData.name,
            culturalSignificance: 'High',
            instruments: countryData.culture?.instruments || []
          });
        });
      }
      
      // Add instruments as well
      if (countryData.culture?.instruments) {
        countryData.culture.instruments.forEach(instrument => {
          music.push({
            name: instrument,
            type: 'Traditional Instrument',
            category: 'Musical Instrument',
            countryCode,
            countryName: countryData.name,
            countryPath: countryCode.toLowerCase(),
            description: `Traditional ${instrument.toLowerCase()} instrument from ${countryData.name}, used in ceremonial and cultural music performances.`,
            origin: countryData.name,
            culturalSignificance: 'High'
          });
        });
  ...
  const categories = useMemo(() => {
    const cats = new Set(allMusic.map(item => item.category));
    return ['All', ...Array.from(cats)];
  }, [allMusic]);

  const countries = useMemo(() => {
    const countryNames = new Set(allMusic.map(item => item.countryName));
    return ['All', ...Array.from(countryNames)];
  }, [allMusic]);

  // Filter music based on search and filters
  const filteredMusic = useMemo(() => {
    return allMusic.filter(item => {
  ...
  }, [allMusic, searchTerm, selectedCategory, selectedCountry]);

  // Get category color
  const getCategoryColor = (category) => {
    const colors = {
      'Traditional Music': 'bg-green-100 text-green-800',
      'Musical Instrument': 'bg-blue-100 text-blue-800',
      'Contemporary Music': 'bg-purple-100 text-purple-800'
    };
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            <div className="flex items-center justify-center mb-6">
              <Music className="w-8 h-8 text-[#A0522D] mr-3" />
              <span className="text-sm font-semibold text-[#A0522D] uppercase tracking-wider">African Musical Heritage</span>
            </div>
            <h1 className="text-5xl md:text-6xl font-bold bg-gradient-to-r from-[#A0522D] to-[#D2691E] bg-clip-text text-transparent mb-6 leading-tight">
              {isCountrySpecific && currentCountryData ? `${currentCountryData.name} Music` : 'All Traditional Music'}
            </h1>
            <p className="text-xl text-gray-600 leading-relaxed mb-8">
              {isCountrySpecific && currentCountryData 
                ? `Discover the musical traditions and rhythmic heritage of ${currentCountryData.name}` 
                : 'Explore the rich musical landscape of Africa - from traditional genres to ancient instruments'
              }
            </p>
            <div className="text-lg text-[#A0522D] font-semibold">
              {filteredMusic.length} musical traditions found
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
            ...
            <div className="flex-1 max-w-md">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search music & instruments..."
                  value={searchTerm}
        ...
        <div className="container mx-auto px-6">
          {filteredMusic.length === 0 ? (
            <div className="text-center py-16">
              ...
              <h3 className="text-2xl font-bold text-[#A0522D] mb-4">No music found</h3>
